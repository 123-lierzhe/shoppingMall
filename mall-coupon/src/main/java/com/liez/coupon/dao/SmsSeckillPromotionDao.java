package com.liez.coupon.dao;

import com.liez.coupon.entity.SmsSeckillPromotion;
import org.apache.ibatis.annotations.Param;

import java.util.List;

/**
 * 秒杀活动(SmsSeckillPromotion)表数据库访问层
 *
 * @author makejava
 * @since 2021-09-07 20:50:22
 */
public interface SmsSeckillPromotionDao {

	/**
	 * 通过ID查询单条数据
	 *
	 * @param id 主键
	 * @return 实例对象
	 */
	SmsSeckillPromotion queryById(Long id);

	/**
	 * 查询指定行数据
	 *
	 * @param offset 查询起始位置
	 * @param limit  查询条数
	 * @return 对象列表
	 */
	List<SmsSeckillPromotion> queryAllByLimit(@Param("offset") int offset, @Param("limit") int limit);


	/**
	 * 通过实体作为筛选条件查询
	 *
	 * @param smsSeckillPromotion 实例对象
	 * @return 对象列表
	 */
	List<SmsSeckillPromotion> queryAll(SmsSeckillPromotion smsSeckillPromotion);

	/**
	 * 新增数据
	 *
	 * @param smsSeckillPromotion 实例对象
	 * @return 影响行数
	 */
	int insert(SmsSeckillPromotion smsSeckillPromotion);

	/**
	 * 批量新增数据（MyBatis原生foreach方法）
	 *
	 * @param entities List<SmsSeckillPromotion> 实例对象列表
	 * @return 影响行数
	 */
	int insertBatch(@Param("entities") List<SmsSeckillPromotion> entities);

	/**
	 * 批量新增或按主键更新数据（MyBatis原生foreach方法）
	 *
	 * @param entities List<SmsSeckillPromotion> 实例对象列表
	 * @return 影响行数
	 */
	int insertOrUpdateBatch(@Param("entities") List<SmsSeckillPromotion> entities);

	/**
	 * 修改数据
	 *
	 * @param smsSeckillPromotion 实例对象
	 * @return 影响行数
	 */
	int update(SmsSeckillPromotion smsSeckillPromotion);

	/**
	 * 通过主键删除数据
	 *
	 * @param id 主键
	 * @return 影响行数
	 */
	int deleteById(Long id);

}

